...
export const participant = new schema.Entity(
  'participants',
  {},
  {
    idAttribute: 'email',
...
const normalizeData = (data) => {
  const normalizedMeetings = normalize(data.reduce((out, { room, meetings }) => out.concat(...meetings.map(meeting => ({ ...meeting, room }))), []), [ meeting ])
  const normalizedRooms = normalize(data.reduce((out, { room }) => out.concat([ room ]), []), [ room ])

  // GAHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
  // But... ehh. Sigh.
  normalizedMeetings.result = normalizedMeetings.result.sort(
    (a, b) => moment(normalizedMeetings.entities.meetings[a].start) - moment(normalizedMeetings.entities.meetings[b].start)
  )

  return {
    meetings: { entities: normalizedMeetings.entities.meetings, result: normalizedMeetings.result },
    participants: { entities: normalizedMeetings.entities.participants, result: Object.keys(normalizedMeetings.entities.participants) },
    rooms: { entities: { ...normalizedMeetings.entities.rooms, ...normalizedRooms.entities.rooms }, result: normalizedRooms.result },
  }
