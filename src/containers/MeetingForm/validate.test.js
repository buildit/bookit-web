...
describe('Meeting form validator', () => {
  const expectedTitleErrorMessage = 'Name is required'
  ...
  it('Should return the proper error if there is no title', () => {
    ...
    expect(validate(formValues)).toEqual({ title: expectedTitleErrorMessage })
  })
  ...
  it('Should return the proper error if the start time is after the end time', () => {
    ...
    expect(validate(formValues)).toEqual({ time: expectedTimeErrorMessage })
  })
